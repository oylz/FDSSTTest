...
void* alMalloc( size_t size, int alignment ) {
	void *re = aligned_alloc(alignment, size);
	return re;
#if 0
  ...
  *(void**) ((size_t) aligned-pSize) = raw;
  return aligned;
#else
void *data = malloc(sizeof(void *)+alignment + size);  
...
#else
void * alMalloc(unsigned int size, unsigned int alignment){  
    ...
    if ((tmp = (unsigned char *) malloc(size + alignment)) != NULL) {  
        mem_ptr =  
            (unsigned char *) ((unsigned int64_t) (tmp + alignment - 1) &  
            (~(unsigned int64_t) (alignment - 1)));  
...
bool fhog3Float( float *M, float *O, float *H, int h, int w, int binSize,
  int nOrients, int softBin, float clip )
{
  ...
  if(sz <= 0){
	...
	VLOG(0) << "wb*hb*nOrients*2:" << (wb*hb*nOrients*2);
...
float* fhog1Float(float* I,int height,int width,int channel,int *h,int *w,int *d,int binSize = 4,int nOrients = 9,float clip=0.2f,bool crop = false){
	VLOG(0) << "height:" << height << "width:" << width;
    ...
    *d = nOrients*3+5;
    ...
    if(!fhog3Float( M, O, H, height, width, binSize, nOrients, -1, clip )){
		delete []M; 
		delete []O;
		delete []H;
        return NULL;
...
float* HOGXYZ(const cv::Mat& input, int &len){
    ...
    int h,w,d;
    float* HH = fhog1Float(I,HEIGHT,WIDTH,DEPTH,&h,&w,&d,binSize,nOrients,clip,crop);
    if(HH == NULL){
		delete []I;
		delete []II;
        return NULL; 
...
cv::Mat fhog(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop){
    ...
    CV_Assert(DEPTH == 1);
	VLOG(0) << "HEIGHT:" << HEIGHT << ", WIDTH:" << WIDTH << ", DEPTH:" << DEPTH << ", binSize:" << binSize;
    float *II = new float[HEIGHT*WIDTH*DEPTH];
    int count = 0;
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
